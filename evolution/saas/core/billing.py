...
class BillingCommandHandler:
    ...
    async def set_plan(
        ...
        try:
            ...
            res_tenant = await data_service.update(
                "tenants", tenant_id, {"plan": plan_id}
            )
            if not res_tenant.success:
                return res_tenant

            # 2. Update or create subscription
            sub_res = await data_service.query(
                "tenant_subscriptions", filters={"tenant_id": tenant_id}, limit=1
            )
        ...
        try:
            ...
            if not sub_res.success or not sub_res.data:
                return ServiceResponse.error_res(
                    "Subscription not found", "SUB_NOT_FOUND"
                )

            sub_id = sub_res.data[0]["id"]
            res = await data_service.update(
                "tenant_subscriptions", sub_id, {"subscription_status": status}
            )
            if not res.success:
                return res
    ...
    async def extend_subscription(
        ...
        try:
            ...
            res = await data_service.update(
                "tenant_subscriptions", sub["id"], {"end_date": new_end_date}
            )
            if not res.success:
                return res
