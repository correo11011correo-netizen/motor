...
def command(
    name: str, description: str = "", params_model: Optional[Dict[str, str]] = None
):
    """
...
class CommandDispatcher:
    def __init__(self, data_service: Optional[DataServiceInterface] = None):
        self.registry: Dict[str, Callable] = {}
        self.data_service = data_service

    def set_data_service(self, data_service: DataServiceInterface) -> None:
        """Inyecta el servicio de datos una vez inicializado el motor."""
        self.data_service = data_service
    ...
    async def execute(
        self, command_name: str, params: dict, context: TenantContext
    ) -> ServiceResponse:
        """
        Executes a business command with PBAC and auditing.
        """
        if not self.data_service:
            logger.error("CommandDispatcher: data_service is not initialized.")
            return ServiceResponse.error_res(
                "System data service not initialized", "SYSTEM_ERROR"
            )

        if command_name not in self.registry:
            logger.warning(f"Command {command_name} not found")
            return ServiceResponse.error_res(
                f"Command {command_name} not found", "CMD_NOT_FOUND"
            )
        ...
        if context.role != "superadmin":
            if required_plan == "pro" and context.plan != "pro":
                return ServiceResponse.error_res(
                    "This command requires a PRO plan.", "PLAN_REQUIRED"
                )
    ...
    async def _audit(self, context: TenantContext, command: str, params: dict) -> None:
        """
        Asynchronous audit record.
        """
        try:
            await self.data_service.insert(
                "audit_log",
                {
