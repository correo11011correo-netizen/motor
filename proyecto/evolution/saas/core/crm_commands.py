...
class CRMCommandHandler:
    ...
    async def create_or_update_customer(
        ...
        try:
            ...
            res_customer = await data_service.query("customers", filters={
            ...
            if res_customer.success and res_customer.data:
                ...
                return ServiceResponse.success_res(
                    data={"customer_id": customer_id}, message="Customer found and updated."
                )

            # Crear nuevo cliente
            customer_id = str(uuid.uuid4())
            res_insert = await data_service.insert("customers", {
                "id": customer_id, 
                "tenant_id": str(context.tenant_id), 
                "phone_number": phone_number, 
                "full_name": full_name, 
            ...
            return ServiceResponse.success_res(
                data={"customer_id": customer_id}, message="Customer created successfully."
            )
    ...
    async def get_customer_profile(
        ...
        try:
            ...
            res_history = await data_service.execute_custom("GET_CUSTOMER_SALES_HISTORY", {
                "tenant_id": str(context.tenant_id),
                "phone_number": phone_number
            })
    ...
    async def list_customers(self, data_service: DataServiceInterface, context: TenantContext) -> ServiceResponse:
        try:
            res = await data_service.query("customers", filters={"tenant_id": str(context.tenant_id)})
            if not res.success:
                return res
